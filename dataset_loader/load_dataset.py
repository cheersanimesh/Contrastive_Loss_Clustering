...
class dataset:
    def __init__(self, dataset):
        ...
        if (dataset == 'cifar_10'):
            ...
            for img in x_train:
                cv2.imwrite(
                    f'{parrent_path}/dataset_dump/train/{dataset}/{ctr}.jpg', img)
                ctr += 1
            ctr = 0
            for img in x_test:
                cv2.imwrite(
                    f'{parrent_path}/dataset_dump/test/{dataset}/{ctr}.jpg', img)
                ctr += 1
        self.training_paths = glob(
            f'{parrent_path}/dataset_dump/train/{dataset}/*.jpg')
        self.testing_paths = glob(
            f'{parrent_path}/dataset_dump/test/{dataset}/*.jpg')
    ...
    def get_train_dataset(self, train_val_split, batch_size):
        ...
        dataset_a = dataset_a.map(
            dataset_operations.read_preprocess_and_augment_image)
        dataset_b = dataset_b.map(
            dataset_operations.read_preprocess_and_augment_image)
    ...
    def get_test_dataset(self, batch_size):
        ...
        test_dataset = test_dataset.map(
            dataset_operations.read_and_preprocess_image)
