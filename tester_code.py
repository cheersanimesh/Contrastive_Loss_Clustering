...
print(sys.path)
'''
directories_list = ['Results', '__pycache__', 'dataset_dump', 'dataset_loader', 'logs',
                    'loss_functions', 'model_architectures', 'model_checkpoints', 'testing_models']
...
training_dataset, val_dataset = dataset.get_train_dataset(
    train_val_split=vars.train_test_split, batch_size=vars.batch_size)

testing_dataset, test_labels = dataset.get_test_dataset(
...
base_cnn_path = "base_cnn 2023-04-09 19:34:45.814411.h5"
print("clustering head path")
##clustering_head_path = str(input())
clustering_head_path = "clustering_head 2023-04-09 19:34:45.814411.h5"
base_cnn_path = os.path.join(parrent_path, 'model_checkpoints', base_cnn_path)
clustering_head_path = os.path.join(
    parrent_path, 'model_checkpoints', clustering_head_path)

base_cnn = keras.models.load_model(base_cnn_path)
clustering_head = keras.models.load_model(clustering_head_path)

print("Testing Model")

tester = testing.testing(
    base_cnn=base_cnn, clustering_head=clustering_head, timestamp=timestamp)
...
logger.logger_multi_write(
    f'Results/Predictions {timestamp} .txt', 'a', ['Predictions --> ', str(predictions)])
logger.logger_multi_write(
    f'Results/Actual Labels {timestamp} .txt', 'a', ['Actual Labels --> ', str(test_labels)])

logger.logger_single_write(
    f'Results/Scores {timestamp} .txt', 'a', f'NMI Score --> {nmi_score} \n')
logger.logger_single_write(
    f'Results/Scores {timestamp} .txt', 'a', f'Rand Score --> {rand_score} \n')
logger.logger_single_write(
    f'Results/Scores {timestamp} .txt', 'a', f'Fmi Score --> {fmi_score} \n')
